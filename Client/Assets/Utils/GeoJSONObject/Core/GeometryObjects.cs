...
namespace GeoJSON {
	...
	public class ArrayArrayGeometryObject : GeometryObject {
		...
		public ArrayArrayGeometryObject(List<JSONObject> jsonObjectList) : base() {
			coordinates = new List<List<PositionObject>> ();
			foreach (JSONObject l in jsonObjectList) {
				List<PositionObject> polygon = new List<PositionObject>();
				coordinates.Add (polygon);
				foreach (JSONObject l2 in l.list) {
					polygon.Add (new PositionObject (l2));
	...
	public class PolygonGeometryObject : ArrayArrayGeometryObject {
		...
		public PolygonGeometryObject(List<JSONObject> jsonObjectList) : base(jsonObjectList) {
		}
	}
	...
	// 	}
	// }
	public class MultiPolygonGeometryObject: GeometryObject {
		public List<PolygonGeometryObject> polygons;
	
		public MultiPolygonGeometryObject(JSONObject jsonObject) : base(jsonObject) {
			polygons = new List<PolygonGeometryObject>();
			foreach(var j in jsonObject["coordinates"].list){
				polygons.Add(new PolygonGeometryObject(j.list));;
			}
		}
	
		protected override JSONObject SerializeGeometry() {
			var polygonsArray = new JSONObject (JSONObject.Type.ARRAY);
			foreach (var polygon in polygons) {
				polygonsArray.Add (polygon.Serialize());
			}
